...
from common.config.config import INSTRUCTION_URL, MOCK_AI, CYODA_AI_API, PROJECT_DIR, REPOSITORY_NAME, CLONE_REPO, \
    REPOSITORY_URL
from entity.app_builder_job.data.data import scheduler_stack, api_request_stack, form_submission_stack, \
    file_upload_stack, external_datasource_stack, workflow_stack, stack, entity_stack
from entity.app_builder_job.workflow.helper_functions import _save_code_to_file, _save_file, _chat, _save_json_entity_to_file, _export_workflow_to_cyoda_ai
...
def init_chats(token, _event):
    if (MOCK_AI=="true"):
        return
    ai_service.init_chat_cyoda(token=token, chat_id=chat_id)
    ai_service.init_workflow_chat(token=token, chat_id=chat_id)


def add_instruction(token, _event):
    ...
    _chat(_event=_event, token=token, ai_endpoint=CYODA_AI_API, chat_id=chat_id)


def add_design_stack(token, _event) -> list:
    design = _chat(_event=_event, token=token, ai_endpoint=CYODA_AI_API, chat_id=chat_id)
    target_dir = 'entity'
    _save_file(json.dumps(design), target_dir, f'app_design.json')
...
def clone_repo(token, _event):
    ...
    os.chdir(clone_dir)
    subprocess.run(["git", "checkout", "-b", str(chat_id)], check=True)
    logger.info(f"Repository cloned to {clone_dir}")


def save_entity_to_file(token, _event) -> str:
    """
...
def export_workflow(token, _event) -> str:
    """
    ...
    data = _save_json_entity_to_file(_event=_event,
                                     ...
                                     chat_id=chat_id,
                                     sub_dir='')
    _export_workflow_to_cyoda_ai(token=token, chat_id=chat_id, data=data)
    return data


def save_processors_code_to_file(token, _event) -> str:
    """
    Save the workflow to a file inside a specific directory.
    """
    entity_name = _event.get("entity").get("entity_name")
    code = _chat(_event=_event, token=token, ai_endpoint=CYODA_AI_API, chat_id=chat_id)
    return _save_code_to_file(entity_name=entity_name, data=code["code"], item='workflow')


def save_logic_code_to_file(token, _event) -> str:
    """
    Save the workflow to a file inside a specific directory.
    """
    entity_name = _event.get("entity").get("entity_name")
    code = _chat(_event=_event, token=token, ai_endpoint=CYODA_AI_API, chat_id=chat_id)
    return _save_code_to_file(entity_name=entity_name, data=code["code"], item='logic')
